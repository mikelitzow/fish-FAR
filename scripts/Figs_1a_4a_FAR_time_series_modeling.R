...
## now extract and plot CMIP projection posteriors by decade
# don't run with plyr loaded!
post <- data.frame()
for(i in 2010:2046){
  
newdata <- data.frame(year_fac=i)

xx <- data.frame(year = i, 
                 posterior = posterior_epred(mod_far_fixef, newdata = newdata, re_formula = NA, resp = "FAR"))
...
post$decade <- ifelse(post$year %in% 2010:2019, "2010s",
                      ifelse(post$year %in% 2020:2029, "2020s",
                             ifelse(post$year %in% 2030:2039, "2030s", "2040s")))
...
g <- ggplot(plot.post, aes(x = posterior, fill = decade)) +
  geom_density(alpha = 0.6, lty = 0) +
...
prop.post <- post %>% 
  ...
  group_by(decade) %>%
  summarize(total = n())
prop.post$count <- c(120000,120000, 120000, 12000*7)
prop.post$proportion <- prop.post$total/prop.post$count

theme_set(theme_bw())
ggplot(prop.post, aes(decade, proportion)) +
  geom_bar(stat="identity", fill=cb[2]) +
  ylab("Proportion of years") +
  theme(axis.title.x = element_blank(),
        axis.text.x = element_text(angle = 30, vjust = 0.5)) 

ggsave("./figs/proportion of good cod years.png", height=3, width=2)
